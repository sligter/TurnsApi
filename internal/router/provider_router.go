...
type RouteRequest struct {
	...
	EnforceModelMappings bool     `json:"enforce_model_mappings,omitempty"`
}
...
func (pr *ProviderRouter) Route(req *RouteRequest) (*RouteResult, error) {
	...
	if req.ProviderGroup != "" {
		...
		if req.ProxyKeyID != "" && pr.proxyKeyManager != nil {
			...
			// 传统的模型匹配路由
			group, groupID = pr.routeByModelWithPermissions(req.Model, req.AllowedGroups, req.EnforceModelMappings)
			if group == nil {
				return nil, fmt.Errorf("no suitable provider group found for model '%s' with current permissions", req.Model)
			}
		}
...
func (pr *ProviderRouter) GetCandidateGroups(req *RouteRequest) ([]string, error) {
	...
	if req.EnforceModelMappings && pr.isMappedOriginalInGroups(req.Model, accessibleGroups) {
		return nil, fmt.Errorf("model '%s' is only available via renamed aliases for the current proxy key", req.Model)
	}
	...
	if req.ForceProviderType != "" {
		groups := pr.GetGroupsByProviderType(req.ForceProviderType, req.AllowedGroups, req.Model, req.EnforceModelMappings)
		if len(groups) == 0 {
	...
	groups := pr.GetGroupsForModel(req.Model, req.AllowedGroups, req.EnforceModelMappings)
	if len(groups) == 0 {
		return nil, fmt.Errorf("no suitable provider group found for model '%s' with current permissions", req.Model)
	}
	return groups, nil
...
func (pr *ProviderRouter) GetGroupsForModel(modelName string, allowedGroups []string, enforceModelMappings bool) []string {
	pr.mutex.RLock()
	...
	if enforceModelMappings && pr.isMappedOriginalInGroups(modelName, accessibleGroups) {
	...
	for _, groupID := range accessibleGroups {
		...
		if !modelSupported {
			for alias, actualModel := range group.ModelMappings {
				if alias == modelName || (!enforceModelMappings && actualModel == modelName) {
					modelSupported = true
					break
				}
			}
...
func (pr *ProviderRouter) GetGroupsByProviderType(providerType string, allowedGroups []string, modelName string, enforceModelMappings bool) []string {
	pr.mutex.RLock()
	defer pr.mutex.RUnlock()
	...
	if enforceModelMappings && pr.isMappedOriginalInGroups(modelName, candidateGroups) {
		return []string{}
	}
...
func (pr *ProviderRouter) isMappedOriginalInGroups(modelName string, groupIDs []string) bool {
	if modelName == "" {
		return false
	}
	...
	for _, groupID := range groupIDs {
		...
		for _, actualModel := range group.ModelMappings {
...
func (pr *ProviderRouter) routeByModelWithPermissions(modelName string, allowedGroups []string, enforceModelMappings bool) (*internal.UserGroup, string) {
	accessibleGroups := pr.getAccessibleGroups(allowedGroups)
	if enforceModelMappings && pr.isMappedOriginalInGroups(modelName, accessibleGroups) {
		return nil, ""
	}
...
func (pr *ProviderRouter) routeByForceProviderType(req *RouteRequest) (*RouteResult, error) {
	candidateGroups := pr.GetGroupsByProviderType(req.ForceProviderType, req.AllowedGroups, req.Model, req.EnforceModelMappings)
	if len(candidateGroups) == 0 {
